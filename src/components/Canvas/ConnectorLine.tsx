...
export const ConnectorLine: React.FC<ConnectorLineProps> = ({
  ...
  const indicatorColor = isLinkActive ? '#34d399' : '#fb7185';
    ...
    <g onClick={handleClick} style={{ cursor: 'pointer' }}>
      <defs>
        <linearGradient id={lineGradId} x1={x1} y1={y1} x2={x2} y2={y2} gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor={isSelected ? '#22d3ee' : 'rgba(148, 163, 184, 0.3)'} />
          <stop offset="50%" stopColor={isSelected ? '#818cf8' : 'rgba(148, 163, 184, 0.2)'} />
          <stop offset="100%" stopColor={isSelected ? '#22d3ee' : 'rgba(148, 163, 184, 0.3)'} />
        </linearGradient>
      </defs>
      ...
      <path
        d={pathData}
        fill="none"
        stroke={isSelected ? 'rgba(255,255,255,0.5)' : 'rgba(34, 211, 238, 0.3)'}
        strokeWidth="1"
      ...
      <circle
        ...
        strokeWidth="0.5"
        style={{ filter: `drop-shadow(0 0 3px ${indicatorColor}60)` }}
      />
      <circle
        cx={p2.x}
        cy={p2.y}
      ...
      {isSelected && (
        <g transform={`translate(${midX - 9}, ${midY - 9})`} onClick={handleDelete} style={{ cursor: 'pointer' }}>
          <circle cx="9" cy="9" r="10" fill="#ef4444" stroke="rgba(0,0,0,0.3)" strokeWidth="0.5"
